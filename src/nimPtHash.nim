...
proc insert(h: GHash; s: pointer; l: uint64) {.importcpp: "#.ins(@)".}
proc build(h: GHash) {.importcpp: "#.build()".}
proc index(h: GHash; s: pointer; l: uint64): CppVector[uint64] {.importcpp: "#.idx(@)".}
...
proc index(h: GHash, s: string): seq[uint64] =
  let
    ac = h.index(s[0].addr, s.len.uint64)
    length = ac.size()
  if length == 0:
    return @[]
...
proc insert(h: GHash; s: string) =
  h.insert(s[0].addr, s.len.uint64)
